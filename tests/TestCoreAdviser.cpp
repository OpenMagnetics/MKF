...
std::vector<Core> load_test_data() {
    auto inventory_path = OpenMagneticsTesting::get_test_data_path(std::source_location::current(), "test_cores.ndjson");
    ...
    while (std::getline(ndjsonFile, jsonLine)) {
        ...
        catch(const CoreShapeNotFoundException& re)
        {
            continue;
...
TEST_CASE("Test_CoreAdviserAvailableCores_All_Cores", "[adviser][core-adviser][available-cores]") {
    clear_databases();
...
TEST_CASE("Test_CoreAdviserAvailableCores_All_Cores_With_Margin", "[adviser][core-adviser][available-cores][smoke-test]") {
    ...
    auto pollutionDegree = PollutionDegree::PD3;
...
TEST_CASE("Test_CoreAdviserAvailableCores_Toroidal_Cores_With_Impedance", "[adviser][core-adviser][available-cores]") {
    clear_databases();
...
TEST_CASE("Test_CoreAdviserAvailableCores_All_Cores_Load_Internally_All", "[adviser][core-adviser][available-cores]") {
    clear_databases();
    double voltagePeakToPeak = 600;
    double dcCurrent = 0;
    double ambientTemperature = 25;
...
TEST_CASE("Test_CoreAdviserAvailableCores_All_Cores_Two_Chosen_Ones", "[adviser][core-adviser][available-cores]") {
    clear_databases();
    double voltagePeakToPeak = 600;
    double dcCurrent = 0;
    double ambientTemperature = 25;
...
TEST_CASE("Test_CoreAdviserAvailableCores_No_Toroids_High_Power", "[adviser][core-adviser][available-cores][smoke-test]") {
    clear_databases();
...
TEST_CASE("Test_CoreAdviserAvailableCores_Only_Toroids_High_Power", "[adviser][core-adviser][available-cores][smoke-test]") {
    clear_databases();
    double voltagePeakToPeak = 600;
    double dcCurrent = 0;
    double ambientTemperature = 25;
    ...
    // REQUIRE(masMagnetics.size() > 0);
    for (auto [mas, scoring] : masMagnetics) {
        REQUIRE(mas.get_mutable_magnetic().get_mutable_core().get_shape_family() == CoreShapeFamily::T);
    }
    settings.reset();
}

TEST_CASE("Test_CoreAdviserAvailableCores_No_Toroids_High_Power_High_Frequency", "[adviser][core-adviser][available-cores]") {
    clear_databases();
...
TEST_CASE("Test_CoreAdviserAvailableCores_No_Toroids_Low_Power", "[adviser][core-adviser][available-cores]") {
    clear_databases();
    double voltagePeakToPeak = 60;
    double dcCurrent = 0;
    double ambientTemperature = 25;
...
TEST_CASE("Test_CoreAdviserAvailableCores_No_Toroids_Low_Power_Low_Losses", "[adviser][core-adviser][available-cores]") {
    clear_databases();
    double voltagePeakToPeak = 60;
    double dcCurrent = 0;
    double ambientTemperature = 25;
    ...
    std::map<CoreAdviser::CoreAdviserFilters, double> weights;
    weights[CoreAdviser::CoreAdviserFilters::COST] = 0.1;
    weights[CoreAdviser::CoreAdviserFilters::EFFICIENCY] = 1;
    weights[CoreAdviser::CoreAdviserFilters::DIMENSIONS] = 1;
...
TEST_CASE("Test_CoreAdviserAvailableCores_No_Toroids_Low_Power_Low_Losses_No_Care_About_Size", "[adviser][core-adviser][available-cores]") {
    clear_databases();
    double voltagePeakToPeak = 60;
    double dcCurrent = 0;
    double ambientTemperature = 25;
    ...
    weights[CoreAdviser::CoreAdviserFilters::COST] = 0.1;
    weights[CoreAdviser::CoreAdviserFilters::EFFICIENCY] = 1;
    weights[CoreAdviser::CoreAdviserFilters::DIMENSIONS] = 0.1;
...
TEST_CASE("Test_CoreAdviserAvailableCores_No_Toroids_Redo_Culling", "[adviser][core-adviser][available-cores][smoke-test]") {
    clear_databases();
    double voltagePeakToPeak = 6000;
    double dcCurrent = 0;
    double ambientTemperature = 25;
    ...
    std::map<CoreAdviser::CoreAdviserFilters, double> weights;
    weights[CoreAdviser::CoreAdviserFilters::COST] = 0.1;
    weights[CoreAdviser::CoreAdviserFilters::EFFICIENCY] = 0.1;
    weights[CoreAdviser::CoreAdviserFilters::DIMENSIONS] = 1;
...
TEST_CASE("Test_CoreAdviserAvailableCores_No_Toroids_Two_Windings", "[adviser][core-adviser][available-cores][smoke-test]") {
    clear_databases();
    double voltagePeakToPeak = 600;
    double dcCurrent = 0;
    double ambientTemperature = 25;
...
TEST_CASE("Test_CoreAdviserAvailableCores_Two_Points_Equal", "[adviser][core-adviser][available-cores][smoke-test]") {
    clear_databases();
    OpenMagnetics::Inputs inputs;
    std::vector<double> turnsRatios = {};
    {
...
TEST_CASE("Test_CoreAdviser_Web_1", "[adviser][core-adviser][available-cores][bug]") {
    clear_databases();
    ...
    auto inventory_path = OpenMagneticsTesting::get_test_data_path(std::source_location::current(), "cores_stock.ndjson");
    ...
    std::vector<Core> cores = json::parse(coresString);

    std::vector<Core> coresInInventory;
    for (auto& core : cores) {
...
TEST_CASE("Test_CoreAdviser_Web_2", "[adviser][core-adviser][available-cores][bug]") {
    clear_databases();
    std::string inputsString = R"({"designRequirements":{"insulation":null,"leakageInductance":null,"magnetizingInductance":{"excludeMaximum":null,"excludeMinimum":null,"maximum":null,"minimum":null,"nominal":1e-05},"market":null,"maximumDimensions":null,"maximumWeight":null,"name":null,"operatingTemperature":null,"strayCapacitance":null,"terminalType":null,"topology":null,"turnsRatios":[]},"operatingPoints":[{"conditions":{"ambientRelativeHumidity":null,"ambientTemperature":42.0,"cooling":null,"name":null},"excitationsPerWinding":[{"current":{"harmonics":{"amplitudes":[2.6732088764802597e-15,3.8218284740811588,1.352034707786142,0.4253304394765631,1.0937048636780878e-15,0.1536120213245562,0.15119524913566243,0.07875264997066994,7.241115656602231e-16,0.047948399955734654,0.05513623287662578,0.03235759328251896,4.090564786249015e-16,0.023392967261331783,0.028681570420819397,0.017771008577657642,2.4672126293013086e-16,0.014016396066471176,0.017807158306087076,0.011386475028370351,1.714592334083309e-16,0.009474348163803117,0.012316262725232965,0.008041926341760328,1.1212044817922766e-16,0.006942375707716124,0.009173279000770123,0.006081161484994412,2.111598525624007e-17,0.00539520003414077,0.007217893995285857,0.004841101358177077,2.7775084398586677e-16,0.004388239086960148,0.005929248667932901,0.004014508967799147,9.486404471810101e-17,0.0037036469670079972,0.005045721135369522,0.0034434847131706546,7.352254058261595e-17,0.0032247860657257685,0.004424650744237962,0.0030404540187279,7.970041183113523e-17,0.002884979494813953,0.003983384372505874,0.0027540516952750064,7.285867231955118e-17,0.0026442785821523935,0.0036719561660329553,0.002552983887311817,1.6777299785974844e-16,0.002478058285030729,0.0034594522045109273,0.002417849611852019,9.246617731272175e-17,0.0023710817878942196,0.0033268345221317414,0.0023367953000158204,1.8921291035788944e-16,0.002314304311501306,0.0032630646073174008,0.002303167019442115],"frequencies":[0.0,103000.0,206000.0,309000.0,412000.0,515000.0,618000.0,721000.0,824000.0,927000.0,1030000.0,1133000.0,1236000.0,1339000.0,1442000.0,1545000.0,1648000.0,1751000.0,1854000.0,1957000.0,2060000.0,2163000.0,2266000.0,2369000.0,2472000.0,2575000.0,2678000.0,2781000.0,2884000.0,2987000.0,3090000.0,3193000.0,3296000.0,3399000.0,3502000.0,3605000.0,3708000.0,3811000.0,3914000.0,4017000.0,4120000.0,4223000.0,4326000.0,4429000.0,4532000.0,4635000.0,4738000.0,4841000.0,4944000.0,5047000.0,5150000.0,5253000.0,5356000.0,5459000.0,5562000.0,5665000.0,5768000.0,5871000.0,5974000.0,6077000.0,6180000.0,6283000.0,6386000.0,6489000.0]},"processed":{"acEffectiveFrequency":131904.75985510723,"average":2.609024107869118e-15,"dutyCycle":0.24999999999999975,"effectiveFrequency":131904.75985510723,"label":"triangular","offset":-4.440892098500626e-16,"peak":4.999999999999999,"peakToPeak":10.0,"rms":2.8876908907560668,"thd":0.3765598442298381},"waveform":{"ancillaryLabel":null,"data":[-5.0,5.0,-5.0],"numberPeriods":null,"time":[0.0,2.4271844660194174e-06,9.70873786407767e-06]}},"frequency":103000.0,"magneticFieldStrength":null,"magneticFluxDensity":null,"magnetizingCurrent":{"harmonics":{"amplitudes":[0.006798274035336369,0.6948836814766789,0.24493767435427322,0.07659321568695032,0.0011851486650484215,0.02713826890868042,0.026338258337802543,0.013524003490593392,0.001185148665048509,0.007934821893277
"frequencies":[0.0,103000.0,206000.0,309000.0,412000.0,515000.0,618000.0,721000.0,824000.0,927000.0,1030000.0,1133000.0,1236000.0,1339000.0,1442000.0,1545000.0,1648000.0,1751000.0,1854000.0,1957000.0,2060000.0,2163000.0,2266000.0,2369000.0,2472000.0,2575000.0,2678000.0,2781000.0,2884000.0,2987000.0,3090000.0,3193000.0,3296000.0,3399000.0,3502000.0,3605000.0,3708000.0,3811000.0,3914000.0,4017000.0,4120000.0,4223000.0,4326000.0,4429000.0,4532000.0,4635000.0,4738000.0,4841000.0,4944000.0,5047000.0,5150000.0,5253000.0,5356000.0,5459000.0,5562000.0,5665000.0,5768000.0,5871000.0,5974000.0,6077000.0,6180000.0,6283000.0,6386000.0,6489000.0]},"processed":{"acEffectiveFrequency":133347.9143873257,"average":-0.006798274035336401,"dutyCycle":0.25,"effectiveFrequency":133342.3181567079,"label":"custom","offset":0.0020903409525253602,"peak":0.9028033263894203,"peakToPeak":1.8014259708737899,"rms":0.5247313861450679,"thd":0.3745218047109},"waveform":{"ancillaryLabel":null,"data":[-0.8417355085620355,-0.8606978872028123,0.9028033263894203,-0.8986226444843696,-0.8417355085620355],"numberPeriods":null,"time":[0.0,7.58495145631068e-08,2.427184466019418e-06,9.632888349514574e-06,9.708737864077681e-06]}},"name":"Primary winding excitation","voltage":{"harmonics":{"amplitudes":[0.078125,4.390192203074813,3.1805418164042503,1.6082999269932894,0.15625,0.7853068032814723,1.0533519383460939,0.747228683429235,0.15625,0.38152854838936,0.6237849662140766,0.5097284946091541,0.15625,0.22395874930175205,0.4366894957016387,0.3969081532881607,0.15625,0.1387657119725448,0.3303628683669758,0.32998229455569084,0.15625,0.08454863298834173,0.2606873758724238,0.2849610019773425,0.15625,0.046391828940503435,0.21067873648230084,0.25205943526842706,0.15625,0.017597888807606342,0.1723953087083574,0.22653191551027138,0.15625,0.005294556301253328,0.14161674515924202,0.20579082584016142,0.15625,0.02426175514926864,0.11588289785500591,0.18829805684603593,0.15625,0.040521304735682134,0.09365264588780098,0.17307466014248524,0.15625,0.05487174035875039,0.07390074623301954,0.1594602594300079,0.15625,0.06786649949748992,0.05590714395539467,0.1469848919856514,0.15625,0.07990999700987583,0.03913858752989208,0.13529576856110245,0.15625,0.09131439305868466,0.023177498052867507,0.1241125131778793,0.15625,0.10233553928853857,0.007676070276481761,0.11319769974196572],"frequencies":[0.0,103000.0,206000.0,309000.0,412000.0,515000.0,618000.0,721000.0,824000.0,927000.0,1030000.0,1133000.0,1236000.0,1339000.0,1442000.0,1545000.0,1648000.0,1751000.0,1854000.0,1957000.0,2060000.0,2163000.0,2266000.0,2369000.0,2472000.0,2575000.0,2678000.0,2781000.0,2884000.0,2987000.0,3090000.0,3193000.0,3296000.0,3399000.0,3502000.0,3605000.0,3708000.0,3811000.0,3914000.0,4017000.0,4120000.0,4223000.0,4326000.0,4429000.0,4532000.0,4635000.0,4738000.0,4841000.0,4944000.0,5047000.0,5150000.0,5253000.0,5356000.0,5459000.0,5562000.0,5665000.0,5768000.0,5871000.0,5974000.0,6077000.0,6180000.0,6283000.0,6386000.0,6489000.0]},"processed":{"acEffectiveFrequency":711072.940860665,"average":-0.078125,"dutyCycle":0.24218749999999975,"effectiveFrequency":711013.8104343135,"label":"rectangular","offset":0.0,"peak":7.5,"peakToPeak":10.0,"rms":4.284784125250653,"thd":0.9507084995254541},"waveform":{"ancillaryLabel":null,"data":[-2.5,7.5,7.5,-2.5,-2.5],"numberPeriods":null,"time":[0.0,0.0,2.4271844660194174e-06,2.4271844660194174e-06,9.70873786407767e-06]}}}],"name":"Operating Point No. 2"}]})";
    std::string weightsString = R"({"COST":0.1,"EFFICIENCY":1,"DIMENSIONS":0.1})";

    std::map<std::string, double> weightsKeysString = json::parse(weightsString);
    std::map<CoreAdviser::CoreAdviserFilters, double> weights;
    for (auto const& pair : weightsKeysString) {
        weights[magic_enum::enum_cast<CoreAdviser::CoreAdviserFilters>(pair.first).value()] = pair.second;
    }
...
TEST_CASE("Test_CoreAdviser_Web_5", "[adviser][core-adviser][available-cores][bug]") {
    clear_databases();
    std::string inputsString = R"({"designRequirements":{"insulation":null,"leakageInductance":null,"magnetizingInductance":{"excludeMaximum":null,"excludeMinimum":null,"maximum":null,"minimum":null,"nominal":1e-05},"market":null,"maximumDimensions":null,"maximumWeight":null,"name":null,"operatingTemperature":null,"strayCapacitance":null,"terminalType":null,"topology":null,"turnsRatios":[]},"operatingPoints":[{"conditions":{"ambientRelativeHumidity":null,"ambientTemperature":42
"frequencies":[0.0,103000.0,206000.0,309000.0,412000.0,515000.0,618000.0,721000.0,824000.0,927000.0,1030000.0,1133000.0,1236000.0,1339000.0,1442000.0,1545000.0,1648000.0,1751000.0,1854000.0,1957000.0,2060000.0,2163000.0,2266000.0,2369000.0,2472000.0,2575000.0,2678000.0,2781000.0,2884000.0,2987000.0,3090000.0,3193000.0,3296000.0,3399000.0,3502000.0,3605000.0,3708000.0,3811000.0,3914000.0,4017000.0,4120000.0,4223000.0,4326000.0,4429000.0,4532000.0,4635000.0,4738000.0,4841000.0,4944000.0,5047000.0,5150000.0,5253000.0,5356000.0,5459000.0,5562000.0,5665000.0,5768000.0,5871000.0,5974000.0,6077000.0,6180000.0,6283000.0,6386000.0,6489000.0]},"processed":{"acEffectiveFrequency":133347.9143873257,"average":-0.006798274035336401,"dutyCycle":0.25,"effectiveFrequency":133342.3181567079,"label":"custom","offset":0.0020903409525253602,"peak":0.9028033263894203,"peakToPeak":1.8014259708737899,"rms":0.5247313861450679,"thd":0.3745218047109},"waveform":{"ancillaryLabel":null,"data":[-0.8417355085620355,-0.8606978872028123,0.9028033263894203,-0.8986226444843696,-0.8417355085620355],"numberPeriods":null,"time":[0.0,7.58495145631068e-08,2.427184466019418e-06,9.632888349514574e-06,9.708737864077681e-06]}},"name":"Primary winding excitation","voltage":{"harmonics":{"amplitudes":[0.078125,4.390192203074813,3.1805418164042503,1.6082999269932894,0.15625,0.7853068032814723,1.0533519383460939,0.747228683429235,0.15625,0.38152854838936,0.6237849662140766,0.5097284946091541,0.15625,0.22395874930175205,0.4366894957016387,0.3969081532881607,0.15625,0.1387657119725448,0.3303628683669758,0.32998229455569084,0.15625,0.08454863298834173,0.2606873758724238,0.2849610019773425,0.15625,0.046391828940503435,0.21067873648230084,0.25205943526842706,0.15625,0.017597888807606342,0.1723953087083574,0.22653191551027138,0.15625,0.005294556301253328,0.14161674515924202,0.20579082584016142,0.15625,0.02426175514926864,0.11588289785500591,0.18829805684603593,0.15625,0.040521304735682134,0.09365264588780098,0.17307466014248524,0.15625,0.05487174035875039,0.07390074623301954,0.1594602594300079,0.15625,0.06786649949748992,0.05590714395539467,0.1469848919856514,0.15625,0.07990999700987583,0.03913858752989208,0.13529576856110245,0.15625,0.09131439305868466,0.023177498052867507,0.1241125131778793,0.15625,0.10233553928853857,0.007676070276481761,0.11319769974196572],"frequencies":[0.0,103000.0,206000.0,309000.0,412000.0,515000.0,618000.0,721000.0,824000.0,927000.0,1030000.0,1133000.0,1236000.0,1339000.0,1442000.0,1545000.0,1648000.0,1751000.0,1854000.0,1957000.0,2060000.0,2163000.0,2266000.0,2369000.0,2472000.0,2575000.0,2678000.0,2781000.0,2884000.0,2987000.0,3090000.0,3193000.0,3296000.0,3399000.0,3502000.0,3605000.0,3708000.0,3811000.0,3914000.0,4017000.0,4120000.0,4223000.0,4326000.0,4429000.0,4532000.0,4635000.0,4738000.0,4841000.0,4944000.0,5047000.0,5150000.0,5253000.0,5356000.0,5459000.0,5562000.0,5665000.0,5768000.0,5871000.0,5974000.0,6077000.0,6180000.0,6283000.0,6386000.0,6489000.0]},"processed":{"acEffectiveFrequency":711072.940860665,"average":-0.078125,"dutyCycle":0.24218749999999975,"effectiveFrequency":711013.8104343135,"label":"rectangular","offset":0.0,"peak":7.5,"peakToPeak":10.0,"rms":4.284784125250653,"thd":0.9507084995254541},"waveform":{"ancillaryLabel":null,"data":[-2.5,7.5,7.5,-2.5,-2.5],"numberPeriods":null,"time":[0.0,0.0,2.4271844660194174e-06,2.4271844660194174e-06,9.70873786407767e-06]}}}],"name":"Operating Point No. 2"}]})";
    std::string weightsString = R"({"COST":1,"EFFICIENCY":1,"DIMENSIONS":1})";
    ...
    for (size_t i = 0; i < masMagnetics.size(); ++i ){
        REQUIRE(bool(masMagnetics[i].first.get_outputs()[0].get_winding_losses().value().get_dc_resistance_per_turn()));
    }

    settings.reset();
}

TEST_CASE("Test_CoreAdviser_Web_6", "[adviser][core-adviser][available-cores][bug][smoke-test]") {
    clear_databases();
...
TEST_CASE("Test_CoreAdviser_Web_7", "[adviser][core-adviser][available-cores][bug]") {
    ...
    int maximumNumberResults = 1;
    bool useOnlyCoresInStock = 0;
    std::string file_path = std::source_location::current().file_name();
    ...
    for (auto const& pair : weightsKeysString) {
        weights[magic_enum::enum_cast<CoreAdviser::CoreAdviserFilters>(pair.first).value()] = pair.second / externalSum;
    }
    ...
    for (auto& masMagnetic : masMagnetics) {
        ...
        result["weightedTotalScoring"] = scoring[name][CoreAdviser::CoreAdviserFilters::COST] + scoring[name][CoreAdviser::CoreAdviserFilters::EFFICIENCY] + scoring[name][CoreAdviser::CoreAdviserFilters::DIMENSIONS];
        result["scoringPerFilter"] = json();
        for (auto& filter : magic_enum::enum_names<CoreAdviser::CoreAdviserFilters>()) {
            std::string filterString(filter);
            result["scoringPerFilter"][filterString] = scoring[name][magic_enum::enum_cast<CoreAdviser::CoreAdviserFilters>(filterString).value()];
        };
...
TEST_CASE("Test_CoreAdviser_Web_8", "[adviser][core-adviser][available-cores][bug]") {
    clear_databases();
    auto json_path_1070 = OpenMagneticsTesting::get_test_data_path(std::source_location::current(), "test_coreadviser_web_8_1070.json");
    std::ifstream json_file_1070(json_path_1070);
    std::string inputsString((std::istreambuf_iterator<char>(json_file_1070)), std::istreambuf_iterator<char>());
    std::string weightsString = R"({"EFFICIENCY":40,"DIMENSIONS":30,"COST":30})";
    int maximumNumberResults = 1;
    bool useOnlyCoresInStock = 0;
    std::string file_path = std::source_location::current().file_name();
...
TEST_CASE("Test_CoreAdviser_User_0", "[adviser][core-adviser][available-cores][bug]") {
    clear_databases();
    ...
    std::string weightsString = R"({"COST":1,"EFFICIENCY":0,"DIMENSIONS":1})";
    std::string file_path = std::source_location::current().file_name();
...
TEST_CASE("Test_CoreAdviser_Web_9", "[adviser][core-adviser][available-cores][bug]") {
    clear_databases();
    auto json_path_1162 = OpenMagneticsTesting::get_test_data_path(std::source_location::current(), "test_coreadviser_web_9_1162.json");
    std::ifstream json_file_1162(json_path_1162);
    std::string inputsString((std::istreambuf_iterator<char>(json_file_1162)), std::istreambuf_iterator<char>());
    std::string weightsString = R"({"EFFICIENCY":40,"DIMENSIONS":30,"COST":30})";
    int maximumNumberResults = 1;
    bool useOnlyCoresInStock = 0;
    std::string file_path = std::source_location::current().file_name();
...
TEST_CASE("Test_CoreAdviserStandardCores_All_Shapes", "[adviser][core-adviser][standard-cores]") {
    settings.reset();
    clear_databases();
    load_core_shapes();
    double voltagePeakToPeak = 600;
...
TEST_CASE("Test_CoreAdviserStandardCores_All_Shapes_Small_Dc_Current", "[adviser][core-adviser][standard-cores]") {
    settings.reset();
    clear_databases();
    load_core_shapes();
    double voltagePeakToPeak = 600;
...
TEST_CASE("Test_CoreAdviserStandardCores_All_Shapes_Medium_Dc_Current", "[adviser][core-adviser][standard-cores]") {
    settings.reset();
    clear_databases();
    load_core_shapes();
    double voltagePeakToPeak = 600;
...
TEST_CASE("Test_CoreAdviserStandardCores_All_Shapes_High_Dc_Current", "[adviser][core-adviser][standard-cores]") {
    settings.reset();
    clear_databases();
    load_core_shapes();
    double voltagePeakToPeak = 600;
...
TEST_CASE("Test_CoreAdviserStandardCores_Only_Toroidal_Shapes", "[adviser][core-adviser][standard-cores]") {
    settings.reset();
    clear_databases();
    load_core_shapes();
    double voltagePeakToPeak = 600;
...
TEST_CASE("Test_CoreAdviserStandardCores_Only_Concentric_Shapes", "[adviser][core-adviser][standard-cores]") {
    settings.reset();
    clear_databases();
    load_core_shapes();
    double voltagePeakToPeak = 600;
    ...
    for (auto [mas, scoring] : masMagnetics) {
        REQUIRE(mas.get_mutable_magnetic().get_mutable_core().get_type() == CoreType::TWO_PIECE_SET);
        REQUIRE(mas.get_mutable_magnetic().get_mutable_core().resolve_material().get_alternatives()->size() > 0);
    }
}

TEST_CASE("Test_CoreAdviserStandardCores_Only_Unique_Shapes", "[adviser][core-adviser][standard-cores]") {
    settings.reset();
    clear_databases();
    load_core_shapes();
    double voltagePeakToPeak = 600;
...
TEST_CASE("Test_CoreAdviserStandardCores_Non_Unique_Shapes", "[adviser][core-adviser][standard-cores]") {
    settings.reset();
    clear_databases();
    load_core_shapes();
    double voltagePeakToPeak = 600;
...
TEST_CASE("Test_CoreAdviserStandardCores_Common_Mode_Choke_Low_Frequency_With_Dc", "[adviser][core-adviser][standard-cores]") {
    settings.reset();
    clear_databases();
    load_core_shapes();
    double voltagePeakToPeak = 600;
...
TEST_CASE("Test_CoreAdviserStandardCores_Common_Mode_Choke_High_Frequency", "[adviser][core-adviser][standard-cores]") {
    settings.reset();
    clear_databases();
    load_core_shapes();
    double voltagePeakToPeak = 600;
...
TEST_CASE("Test_CoreAdviserStandardCores_Planar_Inductor", "[adviser][core-adviser][standard-cores]") {
    settings.reset();
    clear_databases();
    load_core_shapes();
    double voltagePeakToPeak = 600;
...
TEST_CASE("Benchmark_CoreAdviser_Transformer_Turn_Variants_Overhead", "[adviser][core-adviser][benchmark]") {
    ...
    json inputsJson = json::parse(R"({
        "designRequirements": {
            ...
            "topology": "dualActiveBridgeConverter",
            "name": "Benchmark",
            "wiringTechnology": "wound"
        },
        "operatingPoints": [{
