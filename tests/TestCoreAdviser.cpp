...
SUITE(CoreAdviser) {
    ...
    TEST(Test_Only_Toroids_High_Power) {
        ...
        for (auto [mas, scoring] : masMagnetics) {
            ...
            std::cout << mas.get_magnetic().get_core().get_functional_description().get_number_stacks().value() << std::endl; 
            // std::cout << mas.get_magnetic().get_core().get_shape_family() << std::endl; 
            CHECK(mas.get_mutable_magnetic().get_mutable_core().get_shape_family() == OpenMagnetics::CoreShapeFamily::T);
            // CHECK(mas.get_magnetic().get_core().get_functional_description().get_number_stacks() == 1);
        }
        settings->reset();
    }
