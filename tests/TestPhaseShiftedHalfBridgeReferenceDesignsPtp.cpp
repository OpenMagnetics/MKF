// Phase 8h — Golden PtP regression tests for three Pshb (Phase-Shifted
// Full Bridge) reference operating points spanning common ZVS-PSHB
// applications:
//
//   1. Telecom 600 W 12 V brick      — 400 V → 12 V @ 50 A, 100 kHz
//        Full-bridge rectifier.  TI UCC2895 reference, Pressman ch.8.
//   2. Server 1.2 kW 24 V rail       — 400 V → 24 V @ 50 A, 100 kHz
//        Full-bridge rectifier.  TI SLUA571 worked example.
//   3. EV onboard 1 kW 48 V auxiliary — 400 V → 48 V @ 21 A, 100 kHz
//        Full-bridge rectifier.  ON Semi NCP1397 class.
//
// PSHB primary current is a trapezoidal lagging-leg waveform with a
// freewheeling interval where the primary recirculates through the
// rectifier diodes.
//
// NRMSE gate: < 50 %.  The existing Test_Pshb_PtP_AnalyticalVsNgspice
// (TestTopologyPhaseShiftedHalfBridge.cpp:615) gates at < 18 % at a
// single 12 V/50 A operating point; we allow 50 % to absorb wider Vout
// coverage and droop-induced phase-mismatch.
//
// Tags: [converter-model][pshb-topology][refdesign][ptp][slow]
...
void run_ptp_gates(const RefDesignSpec& s) {
    ...
    // Gate 3 — Vout regulation (PSHB input current is bursty bridge-leg
...
TEST_CASE("PSHB reference design PtP — Telecom 600 W 12 V brick "
...
TEST_CASE("PSHB reference design PtP — Server 1.2 kW 24 V rail "
...
TEST_CASE("PSHB reference design PtP — EV onboard 1 kW 48 V auxiliary "
