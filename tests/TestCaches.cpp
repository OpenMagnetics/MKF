...
SUITE(Utils) {
    TEST(Magnetic_Cache_Load) {
        ...
        OpenMagnetics::load_magnetic_in_cache("A", magnetic);
    ...
    TEST(Magnetic_Cache_Compute_Energy) {
        ...
        }
        {
            ...
            OpenMagnetics::MasWrapper mas(masJson);
            auto magnetic = mas.get_magnetic();
            OpenMagnetics::load_magnetic_in_cache("A with different gap", magnetic);
        }
        CHECK(magneticsCache.size() == 2);

        OpenMagnetics::autocomplete_magnetics_in_cache();

        auto references = OpenMagnetics::get_magnetic_cache_references();
        CHECK(references[0] == "A");
        CHECK(references[1] == "A with different gap");
        CHECK(magneticEnergyCache.size() == 0);

        OpenMagnetics::compute_energy_cache();
