...
// =============================================================================
// PLECS exporter tests (merged from TestCircuitSimulatorPlecs.cpp)
// =============================================================================

namespace {
auto plecsOutputFilePath = std::filesystem::path{__FILE__}.parent_path().append("..").append("output");

struct EnsurePlecsOutputDir {
    EnsurePlecsOutputDir() { std::filesystem::create_directories(plecsOutputFilePath); }
};
static EnsurePlecsOutputDir _ensurePlecsOutputDir;
...
TEST_CASE("Test_CircuitSimulatorExporter_Plecs_Simple_Inductor", "[processor][circuit-simulator-exporter][plecs]") {
    ...
    int64_t numberStacks = 1;
    auto gapping = OpenMagneticsTesting::get_ground_gap(0.001);
    auto core = OpenMagneticsTesting::get_quick_core(shapeName, gapping, numberStacks, "N87");
    OpenMagnetics::Magnetic magnetic;
    magnetic.set_core(core);
    magnetic.set_coil(coil);

    auto plecsFile = plecsOutputFilePath;
...
TEST_CASE("Test_CircuitSimulatorExporter_Plecs_Transformer", "[processor][circuit-simulator-exporter][plecs]") {
    ...
    REQUIRE(content.find("MagneticInterface", firstMagInt + 1) != std::string::npos);
