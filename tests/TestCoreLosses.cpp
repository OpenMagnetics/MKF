...
SUITE(CoreLossesAssorted) {
    ...
    TEST(Voltage_And_Current) {
        ...
        OpenMagnetics::MagnetizingInductance magnetizingInductanceModel(std::string{models["gapReluctance"]});
    ...
    TEST(Test_Manufacturer_Magnetec) {
        auto models =
            json::parse(R"({"coreLosses": "PROPRIETARY", "coreTemperature": "MANIKTALA", "gapReluctance": "ZHANG"})");
        auto core = OpenMagnetics::CoreWrapper(json::parse(R"({"functionalDescription": {"gapping": [], "material": "Nanoperm 80000", "numberStacks": 1, "shape": {"magneticCircuit": "closed", "type": "custom", "family": "t", "aliases": [], "name": "T 63/50/30", "dimensions": {"A": {"nominal": 0.063}, "B": {"nominal": 0.050}, "C": {"nominal": 0.030}}}, "type": "toroidal"}, "manufacturerInfo": null, "name": "My Core"})"));
        auto winding = OpenMagnetics::CoilWrapper(json::parse(R"({"bobbin": "Dummy", "functionalDescription": [{"isolationSide": "primary", "name": "Primary", "numberParallels": 1, "numberTurns": 1, "wire": "Dummy"}], "layersDescription": null, "sectionsDescription": null, "turnsDescription": null})"));
        auto operatingPoint = OpenMagnetics::OperatingPoint(json::parse(R"({"conditions": {"ambientRelativeHumidity": null, "ambientTemperature": 25.0, "cooling": null, "name": null}, "excitationsPerWinding": [{"frequency": 100000.0, "magneticFieldStrength": null, "magneticFluxDensity": null, "magnetizingCurrent": null, "name": "My Operating Point", "voltage": {"harmonics": null, "processed": null, "waveform": {"ancillaryLabel": null, "data": [688.5, 688.5, -229.49999999999995, -229.49999999999995, 688.5], "numberPeriods": null, "time": [0.0, 2.4999999999999998e-06, 2.4999999999999998e-06, 1e-05, 1e-05]}}}], "name": null})"));
        ...
        auto magnetizingInductance = OpenMagnetics::resolve_dimensional_values(aux.first.get_magnetizing_inductance());
    ...
    TEST(Test_MagnetizingInductanceModel_From_Excitation) {
        ...
        excitation.set_magnetic_flux_density(magneticFluxDensity);
        double temperature = 100;
        auto magnetizingInductance = OpenMagnetics::CoreLossesLossFactorModel::calculate_magnetizing_inductance_from_excitation(core, excitation, temperature);
    ...
    TEST(Test_CoreLosses_Model_3C95) {
        auto coreLossesModel = OpenMagnetics::CoreLosses().get_core_losses_model("3C95");
    }

    TEST(Test_CoreLosses_Model_Nanoperm_8000) {
        auto coreLossesModel = OpenMagnetics::CoreLosses().get_core_losses_model("Nanoperm 8000");
    }
