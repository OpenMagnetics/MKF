...
TEST_CASE("Test_CoilAdviser_WASM_Replication", "[adviser][coil-adviser][wasm-replication][debug]") {
    ...
    if (masMagneticsWithCoil.size() > 0) {
        auto& resultMas = masMagneticsWithCoil[0];
        auto& coil = resultMas.get_mutable_magnetic().get_mutable_coil();
        auto& magnetic = resultMas.get_mutable_magnetic();
        std::cout << "SUCCESS: Got " << masMagneticsWithCoil.size() << " coil(s)" << std::endl;
        std::cout << std::endl;
        
        // Get winding window dimensions
        auto bobbin = coil.resolve_bobbin();
        auto windingWindow = bobbin.get_winding_window_dimensions();
        double windowWidth = windingWindow[0];
        double windowHeight = windingWindow[1];
        std::cout << "=== Winding Window ===" << std::endl;
        std::cout << "Width: " << windowWidth * 1000 << " mm" << std::endl;
        std::cout << "Height: " << windowHeight * 1000 << " mm" << std::endl;
        std::cout << std::endl;
        
        // Get wires
        auto wires = coil.get_wires();
        
        // Log wire info for each winding
        std::cout << "=== Wire Selection ===" << std::endl;
        for (size_t i = 0; i < coil.get_functional_description().size(); ++i) {
            auto& winding = coil.get_functional_description()[i];
            std::cout << "Winding " << i << " (" << winding.get_name() << "):" << std::endl;
            
            auto& wireUnion = winding.get_wire();
            if (std::holds_alternative<std::string>(wireUnion)) {
                std::string wireName = std::get<std::string>(wireUnion);
                std::cout << "  Wire: " << wireName << std::endl;
                
                // Get wire dimensions
                if (i < wires.size()) {
                    auto& wire = wires[i];
                    if (wire.get_type() == WireType::PLANAR || wire.get_type() == WireType::RECTANGULAR) {
                        if (wire.get_conducting_width()) {
                            double width = resolve_dimensional_values(wire.get_conducting_width().value());
                            std::cout << "  Conducting Width: " << width * 1000 << " mm" << std::endl;
                        }
                        if (wire.get_outer_width()) {
                            double width = resolve_dimensional_values(wire.get_outer_width().value());
                            std::cout << "  Outer Width: " << width * 1000 << " mm" << std::endl;
                        }
                        if (wire.get_conducting_height()) {
                            double height = resolve_dimensional_values(wire.get_conducting_height().value());
                            std::cout << "  Conducting Height: " << height * 1000 << " mm" << std::endl;
                        }
                        if (wire.get_outer_height()) {
                            double height = resolve_dimensional_values(wire.get_outer_height().value());
                            std::cout << "  Outer Height: " << height * 1000 << " mm" << std::endl;
                        }
                    } else if (wire.get_type() == WireType::ROUND) {
                        if (wire.get_conducting_diameter()) {
                            double dia = resolve_dimensional_values(wire.get_conducting_diameter().value());
                            std::cout << "  Conducting Diameter: " << dia * 1000 << " mm" << std::endl;
                        }
                        if (wire.get_outer_diameter()) {
                            double dia = resolve_dimensional_values(wire.get_outer_diameter().value());
                            std::cout << "  Outer Diameter: " << dia * 1000 << " mm" << std::endl;
                        }
                    }
                }
            } else if (std::holds_alternative<OpenMagnetics::Wire>(wireUnion)) {
                auto& wire = std::get<OpenMagnetics::Wire>(wireUnion);
                std::cout << "  Wire: <Wire object>" << std::endl;
                std::cout << "  Wire Type: " << magic_enum::enum_name(wire.get_type()) << std::endl;
                if (wire.get_name()) {
                    std::cout << "  Wire Name: " << wire.get_name().value() << std::endl;
                }
                // Print wire dimensions
                if (wire.get_type() == WireType::PLANAR || wire.get_type() == WireType::RECTANGULAR) {
                    if (wire.get_conducting_width()) {
                        double width = resolve_dimensional_values(wire.get_conducting_width().value());
                        std::cout << "  Conducting Width: " << width * 1000 << " mm" << std::endl;
            ...
            auto numParallels = winding.get_number_parallels();
            std::cout << "  Parallels: " << numParallels << std::endl;
        }
        std::cout << std::endl;
        ...
        if (coil.get_layers_description()) {
            ...
            for (size_t i = 0; i < std::min(size_t(5), layers.size()); ++i) {
                std::cout << "  Layer " << i << ": " << layers[i].get_name() << std::endl;
            }
            if (layers.size() > 5) {
                std::cout << "  ... and " << (layers.size() - 5) << " more layers" << std::endl;
            }
        } else {
            std::cout << "No layer description available" << std::endl;
        }
        std::cout << std::endl;

        // Log turn positions to check if they fit
        std::cout << "=== Turn Positions ===" << std::endl;
        if (coil.get_turns_description()) {
            const auto turns = coil.get_turns_description().value();
            for (size_t i = 0; i < std::min(size_t(5), turns.size()); ++i) {
                std::cout << "Turn " << i << ": (" 
                          << turns[i].get_coordinates()[0] << ", " 
                          << turns[i].get_coordinates()[1] << ", " 
                          << turns[i].get_coordinates()[2] << ")" << std::endl;
            }
            if (turns.size() > 5) {
                std::cout << "... and " << (turns.size() - 5) << " more turns" << std::endl;
            }
        }
        std::cout << std::endl;
        
        // Check if sections and layers fit
        std::cout << "=== Fit Checks ===" << std::endl;
        bool sectionsAndLayersFitting = coil.are_sections_and_layers_fitting();
        std::cout << "Sections and layers fitting: " << (sectionsAndLayersFitting ? "YES" : "NO") << std::endl;
        
        // Check turn collisions and proportions
        bool turnsCheck = OpenMagneticsTesting::check_turns_description(coil);
        std::cout << "Turns check (no collisions, proportions OK): " << (turnsCheck ? "PASS" : "FAIL") << std::endl;
        
        // Check if turns fit within winding window bounds
        bool turnsFitInWindow = true;
        ...
        if (coil.get_turns_description()) {
            ...
            // Get bobbin winding window coordinates
            auto bobbinCoords = bobbin.get_winding_window_coordinates();
            double centerX = bobbinCoords[0];
            double centerY = bobbinCoords[1];
            double halfWidth = windowWidth / 2.0;
            double halfHeight = windowHeight / 2.0;
            
            std::cout << "Turn bounds - X: [" << minX << ", " << maxX << "], Y: [" << minY << ", " << maxY << "]" << std::endl;
            std::cout << "Window bounds - X: [" << (centerX - halfWidth) << ", " << (centerX + halfWidth) 
                      << "], Y: [" << (centerY - halfHeight) << ", " << (centerY + halfHeight) << "]" << std::endl;
            
            // Check if turns are within window (with small tolerance)
            double tolerance = 1e-6;
            if (minX < (centerX - halfWidth - tolerance) || maxX > (centerX + halfWidth + tolerance) ||
                minY < (centerY - halfHeight - tolerance) || maxY > (centerY + halfHeight + tolerance)) {
                turnsFitInWindow = false;
                std::cout << "TURNS OUTSIDE WINDOW BOUNDS!" << std::endl;
            } else {
                std::cout << "All turns within window bounds: YES" << std::endl;
            }
        }
        ...
        std::cout << "Coil fit check: " << (sectionsAndLayersFitting && turnsCheck && turnsFitInWindow ? "PASS" : "FAIL") << std::endl;
        
        REQUIRE(masMagneticsWithCoil.size() > 0);
        CHECK(sectionsAndLayersFitting);
        CHECK(turnsCheck);
        CHECK(turnsFitInWindow);
    } else {
        std::cout << "FAILED: No coil found" << std::endl;
        FAIL("CoilAdviser returned no results");
    }
}
