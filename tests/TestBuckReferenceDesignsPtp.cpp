...
// ── Local copies of the ptp_* helpers used by TestTopologyBuck.cpp ────
// Kept inline so this PtP file is self-contained and can be built /
// run in isolation without dragging in the full topology test file.
...
double ptp_nrmse(const std::vector<double>& ref,
    ...
    // Mean-subtracted, scale-normalized NRMSE with circular phase
    // alignment.  Measures shape similarity only — invariant to DC
    // offset and amplitude scale.  Identical to the helper in
    // TestTopologyBuck.cpp; duplicated here to keep this PtP file
    // self-contained.
    int N = (int)ref.size();
