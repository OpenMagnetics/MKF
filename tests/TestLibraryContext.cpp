...
TEST_CASE("LibraryContext: TypeFilterSet predicate", "[library_context]") {
    TypeFilterSet f;
    REQUIRE(f.empty());
...
TEST_CASE("LibraryContext: enum predicates case-insensitive", "[library_context]") {
    TypeFilterSet f;
    f.allowed = {"etd"};
...
TEST_CASE("LibraryContext: bad JSON throws", "[library_context]") {
    LibraryContext ctx;
...
TEST_CASE("LibraryContext: empty context Scope is a no-op", "[library_context]") {
...
TEST_CASE("LibraryContext: Replace mode wipes only provided kinds", "[library_context]") {
    if (wireDatabase.empty()) load_wires();
...
TEST_CASE("LibraryContext: filterCoresByConstraints respects shape family", "[library_context]") {
...
TEST_CASE("LibraryContext: filterWiresByConstraints respects wire type", "[library_context]") {
    if (wireDatabase.empty()) load_wires();
    AdviserConstraints c;
