...
namespace OpenMagnetics {
    ...
    std::shared_ptr<CoreLossesModel> CoreLossesModel::factory(CoreLossesModels modelName)
    {
        ...
        else throw std::runtime_error("Unknown Core losses mode, available options are: {STEINMETZ, IGSE, BARG, ALBACH, ROSHEN, OUYANG, NSE, MSE}");
    }
    ...
    std::map<std::string, double> CoreLossesSteinmetzModel::get_core_losses(CoreWrapper core, OperationPointExcitation excitation, double temperature)
    {
    ...
    double CoreLossesIGSEModel::get_ki(SteinmetzCoreLossesMethodRangeDatum steinmetzDatum) {
        ...
        std::generate(theta_vect.begin(), theta_vect.end(), [n = 0, &constants]() mutable { return n++ * 2 * std::numbers::pi / constants.number_points_samples_waveforms; });
        double theta_int = 0;

        for (auto& theta : theta_vect) {
            theta_int += pow(fabs(cos(theta)), alpha) * pow(2, beta - alpha) * 2 * std::numbers::pi / constants.number_points_samples_waveforms;
        }
    ...
    std::map<std::string, double> CoreLossesIGSEModel::get_core_losses(CoreWrapper core, OperationPointExcitation excitation, double temperature) {
    ...
    std::map<std::string, double> CoreLossesAlbachModel::get_core_losses(CoreWrapper core, OperationPointExcitation excitation, double temperature) {
        ...
        for (size_t i = 0; i < magneticFluxDensityWaveform.size() - 1; ++i) {
            ...
            equivalentSinusoidalFrequency += pow((magneticFluxDensityWaveform[i + 1] - magneticFluxDensityWaveform[i]) / magneticFluxDensityPeakToPeak, 2) / timeDifference;
        }
    ...
    std::map<std::string, double> CoreLossesMSEModel::get_core_losses(CoreWrapper core, OperationPointExcitation excitation, double temperature) {
        ...
        for (size_t i = 0; i < magneticFluxDensityWaveform.size() - 1; ++i) {
            ...
            equivalentSinusoidalFrequency += pow((magneticFluxDensityWaveform[i + 1] - magneticFluxDensityWaveform[i]) / timeDifference, 2) * timeDifference ;
        }
    ...
    std::map<std::string, double> CoreLossesNSEModel::get_core_losses(CoreWrapper core, OperationPointExcitation excitation, double temperature) {
    ...
    std::map<std::string, double> CoreLossesBargModel::get_core_losses(CoreWrapper core, OperationPointExcitation excitation, double temperature) {
        auto magneticFluxDensity = excitation.get_magnetic_flux_density().value();
        double frequency = excitation.get_frequency();
        magneticFluxDensity = InputsWrapper::standarize_waveform(magneticFluxDensity, frequency);
        auto magneticFluxDensityWaveform = magneticFluxDensity.get_waveform().value().get_data();
    ...
    std::map<std::string, double> CoreLossesRoshenModel::get_core_losses(CoreWrapper core, OperationPointExcitation excitation, double temperature) {
        double effectiveVolume = core.get_processed_description().value().get_effective_parameters().get_effective_volume();
        ...
        if (parameters.count("excessLossesCoefficient")){
            excessEddyCurrentsVolumetricLosses = get_excess_eddy_current_losses_density(excitation, parameters["resistivity"], parameters["excessLossesCoefficient"]);
        }
    ...
    std::map<std::string, double> CoreLossesRoshenModel::get_roshen_parameters(CoreMaterialDataOrNameUnion material, OperationPointExcitation excitation, double temperature) {
        ...
        roshenParameters["coerciveForce"] = coerciveForceData[0].get_magnetic_field() - (coerciveForceData[0].get_temperature() - temperature) * (coerciveForceData[0].get_magnetic_field() - coerciveForceData[1].get_magnetic_field()) / (coerciveForceData[0].get_temperature() - coerciveForceData[1].get_temperature());
        roshenParameters["remanence"] = remanenceData[0].get_magnetic_flux_density() - (remanenceData[0].get_temperature() - temperature) * (remanenceData[0].get_magnetic_flux_density() - remanenceData[1].get_magnetic_flux_density()) / (remanenceData[0].get_temperature() - remanenceData[1].get_temperature());
        roshenParameters["saturationMagneticFieldStrength"] = saturationData[0].get_magnetic_field() - (saturationData[0].get_temperature() - temperature) * (saturationData[0].get_magnetic_field() - saturationData[1].get_magnetic_field()) / (saturationData[0].get_temperature() - saturationData[1].get_temperature());
        roshenParameters["saturationMagneticFluxDensity"] = saturationData[0].get_magnetic_flux_density() - (saturationData[0].get_temperature() - temperature) * (saturationData[0].get_magnetic_flux_density() - saturationData[1].get_magnetic_flux_density()) / (saturationData[0].get_temperature() - saturationData[1].get_temperature());
        if (roshenData.get_coefficients()) {
            ...
            roshenParameters["resistivityFrequencyCoefficient"] = roshenCoefficients.get_resistivity_frequency_coefficient();
            roshenParameters["resistivityMagneticFluxDensityCoefficient"] = roshenCoefficients.get_resistivity_magnetic_flux_density_coefficient();
            roshenParameters["resistivityOffset"] = roshenCoefficients.get_resistivity_offset();
            roshenParameters["resistivityTemperatureCoefficient"] = roshenCoefficients.get_resistivity_temperature_coefficient();
            double resistivity = roshenParameters["resistivityOffset"] + roshenParameters["resistivityTemperatureCoefficient"] * (temperature - 25) + roshenParameters["resistivityMagneticFluxDensityCoefficient"] * magneticFluxDensityAcPeak + roshenParameters["resistivityFrequencyCoefficient"] * frequency;
    ...
    double CoreLossesRoshenModel::get_hysteresis_losses_density(std::map<std::string, double> parameters, OperationPointExcitation excitation) {
        auto constants = Constants();
        ...
        auto magneticFluxDensity = excitation.get_magnetic_flux_density().value();
        double magneticFluxDensityAcPeak = magneticFluxDensity.get_processed().value().get_peak().value() - magneticFluxDensity.get_processed().value().get_offset();
        auto majorLoopParameters = get_major_loop_parameters(saturationMagneticFieldStrength, saturationMagneticFluxDensity, coerciveForce, remanence);
        ...
        for (double i=-saturationMagneticFieldStrength; i <=saturationMagneticFieldStrength; i+=constants.roshen_magnetic_field_strength_step) {
        ...
        auto get_magnetic_flux_density_waveform = [&](std::vector<double> magneticFieldStrength_waveform, bool loop_is_upper=true){
        ...
        while (fabs(magneticFluxDensityDifference) > abs_tol && timeout < 10) {
            size_t mininumPosition = std::distance(std::begin(difference), std::min_element(std::begin(difference), std::end(difference)));
            magneticFluxDensityDifference = fabs(upperMagneticFluxDensityWaveform[mininumPosition]) - magneticFluxDensityAcPeak;
        ...
        for (size_t i=0; i<minimum_length; i++) {
            bhArea += fabs(cutUpperMagneticFluxDensityWaveform[i] - cutLowerMagneticFluxDensityWaveform[i]) * constants.roshen_magnetic_field_strength_step;
        }
    ...
    double CoreLossesRoshenModel::get_eddy_current_losses_density(CoreWrapper core, OperationPointExcitation excitation, double resistivity){
        auto constants = Constants();
        auto magneticFluxDensity = excitation.get_magnetic_flux_density().value();
        double frequency = excitation.get_frequency();
        magneticFluxDensity = InputsWrapper::standarize_waveform(magneticFluxDensity, frequency);
        ...
        for (size_t i = 0; i < magneticFluxDensityWaveform.size() - 1; ++i) {
            ...
            volumetricLossesIntegration += pow((magneticFluxDensityWaveform[i + 1] - magneticFluxDensityWaveform[i]) / timeDifference, 2) * timeDifference;
        }

        double eddyCurrentLossesDensity = centralColumnArea / 8 / std::numbers::pi / resistivity * frequency * volumetricLossesIntegration;

        return eddyCurrentLossesDensity;
    }

    double CoreLossesRoshenModel::get_excess_eddy_current_losses_density(OperationPointExcitation excitation, double resistivity, double alphaTimesN0){
        auto constants = Constants();
        auto magneticFluxDensity = excitation.get_magnetic_flux_density().value();
        double frequency = excitation.get_frequency();
        magneticFluxDensity = InputsWrapper::standarize_waveform(magneticFluxDensity, frequency);
        ...
        for (size_t i = 0; i < magneticFluxDensityWaveform.size() - 1; ++i) {
            ...
            volumetricLossesIntegration += pow(fabs(magneticFluxDensityWaveform[i + 1] - magneticFluxDensityWaveform[i]) / timeDifference, 3 / 2) * timeDifference;
        }
