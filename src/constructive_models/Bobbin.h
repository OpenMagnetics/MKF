...
class Bobbin : public MAS::Bobbin {
  ...
  public:
    ...
    /**
     * The Core overloads take `pinsOrientation` (ABT #1220): absent (the default) builds the
     * bobbin exactly as before, with no pins; VERTICAL or HORIZONTAL also synthesises a two-row
     * THT pinout for that mounting (see synthesise_quick_bobbin_pinout) and stores its pins in
     * processedDescription.pins. Throws when the core cannot carry one (no evidence for the family
     * or orientation, fewer than two pins per row).
     */
    static Bobbin create_quick_bobbin(Core core, bool nullDimensions = false,
                                      std::optional<MAS::OrientationEnum> pinsOrientation = std::nullopt);
    ...
    /**
     * @brief A pinout synthesised for a quick bobbin, with where its rail sits (ABT #1220).
     */
    struct SynthesisedPinout {
        MAS::Pinout pinout;
        double pinRailDistance;     // centre of the main column -> rail outer face, along the pin direction
        std::string rulesClass;     // the src/data/quick_bobbin_pins.json class the rules came from
    };

    /**
     * @brief Two rows of THT pins for a core with no catalogue former (ABT #1220).
     *
     * Every number comes from src/data/quick_bobbin_pins.json, generated by
     * scripts/derive_quick_bobbin_pins.py from the catalogue formers that state their pin geometry
     * (medians over distinct footprints), plus the IEC 62317 2.54 mm grid:
     *   pitch        the band of the core's family class that holds core.get_width();
     *   pins per row as many as fit along the core width less one edge margin per end, floor
     *                ((width - 2 margin) / pitch) + 1, the same on both rows;
     *   rowDistance  the smallest grid multiple placing each row the class's clearance beyond the
     *                core depth (VERTICAL: rows straddle Z) or the window height (HORIZONTAL: one
     *                row per end flange, along Y);
     *   rail         the core's outer face along the pin direction (height/2 for VERTICAL, depth/2
     *                for HORIZONTAL) plus the rail standoff, so the pins clear the core by construction;
     *   pin          round THT, catalogue median diameter and length.
     * Nothing in MAS can mark a processed-only bobbin as synthesised (it has no functionalDescription
     * to carry a pinout or a subtype, and pins[] has no provenance field), so the caller knows it by
     * having asked for it.
     *
     * @throws InvalidInputException when the core's family has no class in the table, the class has
     *         no catalogue former of that orientation, or fewer than the table's minimum pins fit a row.
     */
    static SynthesisedPinout synthesise_quick_bobbin_pinout(const Core& core, MAS::OrientationEnum orientation);
    /**
