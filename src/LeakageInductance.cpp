...
LeakageInductanceOutput LeakageInductance::calculate_leakage_inductance(OperatingPoint operatingPoint, MagneticWrapper magnetic, size_t sourceIndex, size_t destinationIndex, size_t harmonicIndex) {
    auto settings = OpenMagnetics::Settings::GetInstance();
    auto numberPointsX = settings->get_magnetic_field_number_points_x();
    auto numberPointsY = settings->get_magnetic_field_number_points_y();
    ...
    ComplexField field = calculate_magnetic_field(operatingPoint, magnetic, harmonicIndex);
