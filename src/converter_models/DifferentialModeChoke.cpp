...
namespace OpenMagnetics {
    ...
    DifferentialModeChoke::DifferentialModeChoke(const json& j) {
        // Required field guard before delegating to MAS::from_json (which would
        // throw a less specific message via j.at()).
        ...
        if (!j.contains("inputVoltage")) {
            throw std::runtime_error("DifferentialModeChoke: 'inputVoltage' is required");
        }
        if (!j.contains("operatingCurrent")) {
            throw std::runtime_error("DifferentialModeChoke: 'operatingCurrent' is required");
        }
        if (!j.contains("ambientTemperature")) {
            throw std::runtime_error("DifferentialModeChoke: 'ambientTemperature' is required");
        }

        // Delegate to the MAS-generated from_json, which handles every spec
        // field (including the Configuration enum mapping from the schema
        // strings singlePhase / singlePhaseBalanced / threePhase /
        // threePhaseWithNeutral) and the optional vectors.
        MAS::from_json(j, static_cast<MAS::DifferentialModeChoke&>(*this));
    }
    ...
    DesignRequirements DifferentialModeChoke::process_design_requirements() {
        ...
        auto minInductance = get_minimum_inductance();
        if (minImpedance) {
            designRequirements.set_minimum_impedance(minImpedance.value());
        ...
        else if (minInductance) {
            DimensionWithTolerance inductanceWithTolerance;
            inductanceWithTolerance.set_minimum(minInductance.value());
            designRequirements.set_magnetizing_inductance(inductanceWithTolerance);
        }
    ...
    std::vector<OperatingPoint> DifferentialModeChoke::process_operating_points() {
        ...
        if (currentRipple < 0) {
            currentRipple = operatingCurrent * 0.2;  // Default 20% ripple
        }

        int numWindings = get_number_of_windings();
        auto inputVoltage = get_input_voltage();
        double operatingVoltage = resolve_dimensional_values(inputVoltage);
        auto configuration = get_configuration_or_default();
        ...
        for (int windingIdx = 0; windingIdx < numWindings; windingIdx++) {
            ...
            bool isNeutral = (configuration == DmcConfiguration::THREE_PHASE_WITH_NEUTRAL &&
                             windingIdx == 3);
            ...
            double currentAmplitude = operatingCurrent * std::sqrt(2);  // RMS to peak
            if (isNeutral) {
    ...
    std::string DifferentialModeChoke::generate_ngspice_circuit(double inductance, double frequency) {
        ...
        auto inputVoltage = get_input_voltage();
        double operatingVoltage = resolve_dimensional_values(inputVoltage);
        double loadResistance = operatingVoltage / get_operating_current();
    ...
    std::vector<OperatingPoint> DifferentialModeChoke::simulate_and_extract_operating_points(double inductance) {
        ...
        auto minImpedance = get_minimum_impedance();
        if (minImpedance.has_value()) {
            for (const auto& impReq : minImpedance.value()) {
    ...
    std::vector<DmcAttenuationResult> DifferentialModeChoke::verify_attenuation(
        ...
        double filterCapacitance = capacitance.value_or(get_filter_capacitance().value_or(0));
    ...
    json DifferentialModeChoke::propose_design() {
        ...
        auto minImpedance = get_minimum_impedance();
        if (minImpedance.has_value() && !minImpedance->empty()) {
            // Use first impedance requirement as primary target
            const auto& impReq = minImpedance->front();
        ...
        double capacitance = get_filter_capacitance().value_or(0);
        ...
        auto minInductance = get_minimum_inductance();
        if (minInductance.has_value() && minInductance.value() > inductance) {
            inductance = minInductance.value();
        ...
        design["configuration"] = configuration == DmcConfiguration::SINGLE_PHASE ? "singlePhase" :
                                  configuration == DmcConfiguration::SINGLE_PHASE_BALANCED ? "singlePhaseBalanced" :
                                  configuration == DmcConfiguration::THREE_PHASE ? "threePhase" :
