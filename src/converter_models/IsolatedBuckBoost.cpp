...
namespace OpenMagnetics {
    ...
    std::vector<IsolatedBuckBoostTopologyWaveforms> IsolatedBuckBoost::simulate_and_extract_topology_waveforms(
        ...
        std::vector<IsolatedBuckBoostTopologyWaveforms> allWaveforms;
        ...
        for (size_t inputVoltageIndex = 0; inputVoltageIndex < inputVoltages.size(); ++inputVoltageIndex) {
            ...
            for (size_t opIndex = 0; opIndex < get_operating_points().size(); ++opIndex) {
                ...
                double switchingFrequency = ibbOpPoint.get_switching_frequency();
                double primaryOutputVoltage = ibbOpPoint.get_output_voltages()[0];
                double efficiency = 1;
                if (get_efficiency()) {
                    efficiency = get_efficiency().value();
                }
                ...
                std::cout << "DEBUG [IsolatedBuckBoost]: Setting config.steadyStateCycles = " << config.steadyStateCycles << std::endl;
                config.keepTempFiles = false;
                ...
                waveforms.dutyCycle = calculate_duty_cycle(inputVoltage, primaryOutputVoltage, efficiency);
                
                // Extract waveforms by name from the simulation result
                // Build a helper to strip v() or i() prefixes from ngspice names
                auto stripPrefix = [](const std::string& name) -> std::string {
                    ...
                    if (lower.size() > 2 && (lower[0] == 'v' || lower[0] == 'i') && lower[1] == '(') {
                        ...
                        if (closeParenPos != std::string::npos && closeParenPos > 2) {
                            return lower.substr(2, closeParenPos - 2);
                        }
                    }
                    return lower;
                };
                ...
                for (size_t i = 0; i < simResult.waveformNames.size(); ++i) {
                    const auto& rawName = simResult.waveformNames[i];
                    ...
                    } else if (name == "vpri_sense" || name == "vpri_sense#branch") {
                        waveforms.primaryCurrent = waveform.get_data();
                    } else if (name == "vpri_out") {
                        waveforms.outputVoltage = waveform.get_data();
                    } else if (name == "vpri_out_sense" || name == "vpri_out_sense#branch") {
                        waveforms.outputCurrent = waveform.get_data();
                    }
                    
                    for (size_t secIdx = 0; secIdx < numSecondaries; ++secIdx) {
                        // Use output voltage vout0, vout1, etc. instead of winding voltage sec0_in, sec1_in
                        if (name == "vout" + std::to_string(secIdx)) {
                        ...
                        } else if (name == "vsec_sense" + std::to_string(secIdx) || 
                                   name == "vsec_sense" + std::to_string(secIdx) + "#branch") {
