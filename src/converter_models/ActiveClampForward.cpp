...
namespace OpenMagnetics {
    ...
    OperatingPoint ActiveClampForward::process_operating_points_for_input_voltage(double inputVoltage, const ForwardOperatingPoint& outputOperatingPoint, const std::vector<double>& turnsRatios, double inductance, double mainOutputInductance) {
        ...
        // Active-Clamp Forward has NO inherent /2 (unlike push-pull/half-
        // bridge where D is per-half-cycle); the clamp switch handles reset
        // during 1−D, not during a separate half-period.  Matches the SPICE
        // netlist tOn computation so SPICE and analytical use the same duty
        // cycle and waveform timing.
        ...
        if (minimumPrimaryCurrent < 0) { // DCM
            ...
            if (sqrtArg < 0) {
                throw InvalidInputException(ErrorCode::INVALID_DESIGN_REQUIREMENTS, "ActiveClampForward: negative value under sqrt in DCM t1 calculation");
            }
            t1 = sqrt(sqrtArg);
            if (t1 > period / 2) {
                throw InvalidInputException(ErrorCode::INVALID_DESIGN_REQUIREMENTS, "T1 cannot be larger than period/2, wrong topology configuration");
        ...
        double denom = 1 - t1 * switchingFrequency;
        if (fabs(denom) < 1e-12) {
            throw InvalidInputException(ErrorCode::INVALID_DESIGN_REQUIREMENTS, "ActiveClampForward: clamp voltage undefined when duty cycle equals 1");
        }
        auto clampVoltage = t1 * switchingFrequency / denom * inputVoltage;
    ...
    DesignRequirements ActiveClampForward::process_design_requirements() {
        ...
        // ACF clamp capacitor steady-state voltage = D/(1-D) * Vin_min — the
        // canonical Erickson Active-Clamp Reset analysis (Section 6.4.2).
        if (dutyCycle < 1.0) {
            lastClampCapVoltage = dutyCycle / (1.0 - dutyCycle) * minimumInputVoltage;
        }
    ...
    std::vector<OperatingPoint> ActiveClampForward::process_operating_points(const std::vector<double>& turnsRatios, double magnetizingInductance) {
        ...
        extraClampCapVoltageWaveforms.clear();
        extraClampVoltages.clear();
        extraSwitchingFrequencies.clear();

        std::vector<double> outputInductancePerSecondary;

        size_t numSecondaries = turnsRatios.size();  // ActiveClampForward has no demag winding
        ...
        for (size_t inputVoltageIndex = 0; inputVoltageIndex < inputVoltages.size(); ++inputVoltageIndex) {
            ...
            for (size_t forwardOperatingPointIndex = 0; forwardOperatingPointIndex < get_operating_points().size(); ++forwardOperatingPointIndex) {
                ...
                actualDutyCycle = std::min(actualDutyCycle, dutyCycle);
                double tOn = actualDutyCycle * period;

                // Clamp voltage = D/(1-D) * Vin
                double denom = 1.0 - actualDutyCycle;
                double clampVoltage = (denom > 1e-12) ? (actualDutyCycle / denom * inputVoltage) : inputVoltage;
                extraClampVoltages.push_back(clampVoltage);
                extraSwitchingFrequencies.push_back(switchingFrequency);

                // Clamp cap voltage waveform (rectangular: clampVoltage during on-time, 0 during off-time)
                Waveform clampVoltWfm;
                clampVoltWfm.set_data(std::vector<double>{clampVoltage, clampVoltage, 0, 0});
                clampVoltWfm.set_time(std::vector<double>{0, tOn, tOn, period});
                clampVoltWfm.set_ancillary_label(WaveformLabel::CUSTOM);
                extraClampCapVoltageWaveforms.push_back(clampVoltWfm);
    ...
    DesignRequirements AdvancedActiveClampForward::process_design_requirements() {
        // Issue M1 override: build DR from desired* fields directly.
    ...
    std::string ActiveClampForward::generate_ngspice_circuit(
    ...
    collect_input_voltages(get_input_voltage(), inputVoltages, inputVoltagesNames_);
        ...
        circuit << "* Active Clamp Forward Converter - Generated by OpenMagnetics\n";
        ...
        // drain. Active-Clamp Forward has S1 (main) + S_clamp; only S1
        // sources current from the DC bus (S_clamp swaps energy between
        // the clamp cap and the magnetising branch and does NOT add to
        // the bus draw), so a single ammeter on S1 captures the
        // converter's input current. Sign: + terminal is vin_dc, - is
        ...
        circuit << ".save v(pri_in) i(Vpri_sense) v(vin_dc) i(Vin) i(Vq1_sense)";
        for (size_t secIdx = 0; secIdx < numSecondaries; ++secIdx) {
    ...
    std::vector<OperatingPoint> ActiveClampForward::simulate_and_extract_operating_points(
        const std::vector<double>& turnsRatios,
        double magnetizingInductance) {
    ...
    std::vector<ConverterWaveforms> ActiveClampForward::simulate_and_extract_topology_waveforms(const std::vector<double>& turnsRatios,
        double magnetizingInductance,
    ...
    for (size_t inputVoltageIndex = 0; inputVoltageIndex < inputVoltages.size(); ++inputVoltageIndex) {
        for (size_t opIndex = 0; opIndex < get_operating_points().size(); ++opIndex) {
            ...
            //     upstream of S1's drain. Active-Clamp Forward has S1
            //     (main) + S_clamp; S_clamp only swaps energy between
            //     the clamp cap and the magnetising branch and does NOT
            //     draw from vin_dc, so a single ammeter on S1 captures
            //     the converter's draw. Note the primary winding
            //     voltage that appears during off-time has a
            //     clamp-capacitor component, but the differential
            //     winding probe (pri_in -> 0 — Lpri is already
            //     winding-only) filters that into the true winding
            //     voltage; no Evpri_w E-source is needed here (Lpri is
            //     wired pri_in -> 0, secondary Lsec<i> wired sec<i>_in
            //     -> 0). Sign: + terminal of Vq1_sense is vin_dc, so
            //     positive i(Vq1_sense) is current flowing OUT of
            //     vin_dc INTO S1 (non-negative in CCM/DCM).
            wf.set_input_voltage(getWaveform("vin_dc"));

            Waveform iIn = getWaveform("vq1_sense#branch");
            if (iIn.get_data().empty()) {
                throw std::runtime_error(
                    "ActiveClampForward simulate_and_extract_topology_waveforms: "
            ...
            // ActiveClampForward has no demag winding — every entry in
    ...
    std::vector<std::variant<Inputs, CAS::Inputs>> ActiveClampForward::get_extra_components_inputs(
        ExtraComponentsMode /*mode*/,
        std::optional<Magnetic> /*magnetic*/)
    {
        if (extraLoInductances.empty() || extraLoVoltageWaveforms.empty())
            throw std::runtime_error("ActiveClampForward::get_extra_components_inputs: call process_operating_points() first");
        ...
        // Output inductors (one per secondary)
        for (size_t s = 0; s < numSecondaries; ++s) {
            double lo = extraLoInductances[s];
            if (lo <= 0)
                throw std::runtime_error("ActiveClampForward::get_extra_components_inputs: computedOutputInductance <= 0 for secondary " + std::to_string(s));
        ...
        // Clamp capacitor CAS::Inputs
        {
            CAS::Inputs casInputs;
            CAS::DesignRequirements dr;

            // Peak clamp voltage across all operating points
            double peakClampVoltage = 0.0;
            for (double vc : extraClampVoltages)
                peakClampVoltage = std::max(peakClampVoltage, vc);
            ...
            for (size_t opIdx = 0; opIdx < extraClampCapVoltageWaveforms.size(); ++opIdx) {
                CAS::TwoTerminalOperatingPoint op;
                CAS::OperatingPointExcitation excitation;
                excitation.set_frequency(extraSwitchingFrequencies[opIdx % extraSwitchingFrequencies.size()]);

                CAS::SignalDescriptor vSig;
                CAS::Waveform vWfm;
                vWfm.set_data(extraClampCapVoltageWaveforms[opIdx].get_data());
                if (extraClampCapVoltageWaveforms[opIdx].get_time())
                    vWfm.set_time(extraClampCapVoltageWaveforms[opIdx].get_time().value());
