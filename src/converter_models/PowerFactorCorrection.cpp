...
namespace OpenMagnetics {
    ...
    std::string PowerFactorCorrection::generate_ngspice_switching_circuit(
        ...
        validate_topology_variant();
        const auto variant = get_topology_variant_or_default();
        // SEPIC/Ćuk (buck-boost class): CCM is intractable for a switching PtP
        // (Cc-L2 resonance — handoff §2b), but DCM is resonance-free and
        // self-shaping. Route a DCM SEPIC/Ćuk to the voltage-mode VOT branch.
        if (is_buck_boost_class()) {
            auto m = MAS::PowerFactorCorrection::get_mode();
            if (m.has_value() &&
                m.value() == PfcModes::DISCONTINUOUS_CONDUCTION_MODE) {
                return generate_ngspice_switching_circuit_dcm_sepic(
                    inductance, numberOfLineCycles);
            }
        }
    ...
    std::string PowerFactorCorrection::generate_ngspice_switching_circuit_dcm_sepic(
            double inductance,
            int numberOfLineCycles) {
        const auto cfg = spice_config();
        if (!is_buck_boost_class()) {
            throw std::runtime_error(
                "generate_ngspice_switching_circuit_dcm_sepic: only SEPIC/Ćuk "
                "(buck-boost class) are valid here");
        }
        if (!std::isfinite(inductance) || inductance <= 0.0) {
            throw std::invalid_argument(
                "generate_ngspice_switching_circuit_dcm_sepic: inductance must "
                "be positive and finite (got " + std::to_string(inductance) + ")");
        }
        if (numberOfLineCycles < 1) numberOfLineCycles = 1;
        ...
        const double fsw     = get_switching_frequency();
        const double fline   = get_line_frequency_required();
        const double eta     = get_efficiency_required();
        if (pout <= 0.0) {
            throw std::invalid_argument(
                "generate_ngspice_switching_circuit_dcm_sepic: output power must "
                "be positive (got " + std::to_string(pout) + ")");
        }
        auto bulkCapOpt = MAS::PowerFactorCorrection::get_bulk_capacitance();
        if (!bulkCapOpt.has_value()) {
            throw std::runtime_error(
                "generate_ngspice_switching_circuit_dcm_sepic: bulkCapacitance "
                "is required (set via set_bulk_capacitance() or JSON).");
        }
        const double cbus  = bulkCapOpt.value();
        const double vpk   = std::sqrt(2.0) * vrmsNom;
        const double rload = vbusNom * vbusNom / pout;
        ...
        const double l1    = inductance;                 // input inductor (= 2·Le)

        // ── SEPIC passive sizing (coupled, L1 = L2) ─────────────────────
        // Place the Cc-L resonance a few × below fsw and damp it with a modest
        // series ESR. These exact rules reproduce the session-validated design
        // (L1=L2=200 µH → Cc≈317 nF, Resr≈5 Ω) which gave PF≈0.96 / THD≈10 %.
        const double l2   = l1;
        const double cc   = 1.0 / (std::pow(2.0 * M_PI * (fsw / 5.0), 2) * l2);
        const double resr = 0.2 * std::sqrt(l2 / cc);

        // ── Voltage loop (type-II), reused from the boost design note ────
        const double vref   = 2.5;
        const double k_div  = vref / vbusNom;
        const double gv_fc  = std::max(5.0, fline / 5.0);
        const double gv_rz  = 100e3;
        const double gv_cz  = 1.0 / (2.0 * M_PI * (gv_fc / 3.0) * gv_rz);
        const double gv_cp  = 1.0 / (2.0 * M_PI * (3.0 * gv_fc) * gv_rz);
        const double vea_min = 0.1, vea_max = 4.9, ic_vea = 2.5;

        // Warm-start duty: i_in_avg = Vin·d_nom²/(2·L1·fsw) ∝ Vin (unity PF with
        // VOT), envelope peak = √2·Pin/Vrms ⇒ d_nom = √(2·L1·fsw·Pin)/Vrms.
        const double d_nom = std::sqrt(2.0 * l1 * fsw * pin) / vrmsNom;

        std::ostringstream c;
        c.imbue(std::locale::classic());
        const char* variantLabel =
            (get_topology_variant_or_default() == PfcTopologyVariants::CUK)
                ? "Ćuk" : "SEPIC";
        c << "* PFC " << variantLabel << " DCM Switching Converter "
             "(voltage-mode variable-on-time controller)\n";
        c << "* Generated by OpenMagnetics — see PFC_TOPOLOGY_HANDOFF.md §2b\n";
        c << "* Vrms=" << vrmsNom << " V, Vbus=" << vbusNom << " V, Pout=" << pout
          << " W, Fsw=" << (fsw/1e3) << " kHz, fline=" << fline
          << " Hz, L1=L2=" << (l1*1e6) << " uH, Cc=" << (cc*1e9)
          << " nF, Cbus=" << (cbus*1e6) << " uF\n\n";
        ...
        c << ".param l2="     << sci(l2)     << "\n";
        c << ".param cc="     << sci(cc)     << "\n";
        c << ".param resr="   << sci(resr)   << "\n";
        c << ".param cbus="   << sci(cbus)   << "\n";
        c << ".param rload="  << sci(rload)  << "\n\n";
        c << ".param vref="   << sci(vref)   << "\n";
        c << ".param k_div="  << sci(k_div)  << "\n";
        c << ".param gv_rz="  << sci(gv_rz)  << "\n";
        c << ".param gv_cz="  << sci(gv_cz)  << "\n";
        c << ".param gv_cp="  << sci(gv_cp)  << "\n";
        c << ".param vea_min="<< sci(vea_min)<< "\n";
        c << ".param vea_max="<< sci(vea_max)<< "\n";
        c << ".param d_nom="  << sci(d_nom)  << "\n";
        c << ".param ic_vbus="<< sci(vbusNom)<< "\n";
        c << ".param ic_vea=" << sci(ic_vea) << "\n\n";

        // ── Subckt prelude ──────────────────────────────────────────────
        c << spice_subckt_prelude_pfc_controller() << "\n";

        // ── Power stage (bridged SEPIC, DCM) ────────────────────────────
        c << "* ─── Power stage (SEPIC, DCM) ──────────────────────────────\n";
        c << "B_vin     vin_rect 0  V=vpk*abs(sin(2*3.141592653589793*fline*time))\n";
        c << "Vl_sense  vin_rect na 0          ; input-current sense (= iL1)\n";
        c << ".model SW1 SW (VT=" << cfg.swModelVT << " VH=" << cfg.swModelVH
          << " RON=" << cfg.swModelRON << " ROFF=" << cfg.swModelROFF << ")\n";
        c << ".model DIDEAL D (IS=" << std::scientific << cfg.diodeIS
          << " RS=" << cfg.diodeRS << std::defaultfloat;
        if (!cfg.diodeExtra.empty()) c << " " << cfg.diodeExtra;
        c << ")\n";
        c << "D_br      na nl1 DIDEAL          ; input bridge (blocks reverse iL1)\n";
        c << "L1        nl1 sw {l1}\n";
        c << "S1        sw 0 gate 0 SW1\n";
        c << "Rsnub_s1  sw snub_n " << cfg.snubR << "\n";
        c << "Csnub_s1  snub_n 0 " << std::scientific << cfg.snubC
          << std::defaultfloat << "\n";
        c << "Cc        sw ccx {cc}\n";
        c << "Rcc       ccx nb {resr}          ; Cc ESR (damps the SEPIC resonance)\n";
        c << "L2        nb 0 {l2}\n";
        c << "D1        nb vbus DIDEAL\n";
        c << "Cout      vbus 0 {cbus} IC={ic_vbus}\n";
        c << "Rload     vbus 0 {rload}\n\n";

        // ── Voltage error amp (type-II) ─────────────────────────────────
        c << "* ─── Voltage error amp (Gv) — type-II ─────────────────────\n";
        c << "B_div     vbus_div 0  V=V(vbus)*k_div\n";
        c << "R_in_v    vbus_div vea_n {gv_rz}\n";
        c << "R_fb_z    vea_n vea_z {gv_rz}\n";
        c << "C_fb_z    vea_z vea {gv_cz} IC=0\n";
        c << "C_fb_p    vea_n vea {gv_cp} IC=0\n";
        c << "X_op_v    vref_node vea_n vea OPAMP_IDEAL "
             "params: A0=1e3 GBW=10e6 VSSPOS={vea_max} VSSNEG={vea_min} IC_FILT={ic_vea}\n";
        c << "V_vref    vref_node 0 {vref}\n\n";

        // ── Variable-on-time modulator ──────────────────────────────────
        // vea (~2.5, boost-like so the EA feedback-cap ICs stay consistent) is
        // SCALED to the duty and shaped by the VOT feed-forward
        //   dctrl = vea·(d_nom/2.5) / √(1 + Vin/Vo)
        // The 1/√(1+Vin/Vo) cancels the DCM-COT (1+Vin/Vo) input-current
        // distortion → i_in_avg ∝ Vin (unity PF). gate fires while dctrl > saw.
        c << "* ─── Variable-on-time modulator (DCM self-shaping + VOT FF) ─\n";
        c << "V_saw     saw 0 PULSE(0 1 0 {tsw-1e-9} 1e-9 1e-9 {tsw})\n";
        c << "B_dctrl   dctrl 0 V=V(vea)*(d_nom/2.5)/"
             "sqrt(1+V(vin_rect)/max(V(vbus),1))\n";
        c << "B_gate    gate 0  V=(V(dctrl) > V(saw)) ? 5 : 0\n\n";

        // ── ICs + analysis ──────────────────────────────────────────────
        c << ".ic v(vbus)={ic_vbus}\n\n";
        const double simTime = numberOfLineCycles * tline;
        const double maxStep = tsw / 20.0;
        c << ".tran " << sci(maxStep) << " " << sci(simTime) << " 0 "
          << sci(maxStep) << " uic\n";
        c << ".save i(vl_sense) v(vin_rect) v(vbus) v(vea) v(dctrl)\n";
        c << ".options METHOD=" << cfg.method << " TRTOL=" << cfg.trTol
          << " RELTOL=" << cfg.relTol
          << " ABSTOL=" << std::scientific << cfg.absTol
          << " VNTOL=" << cfg.vnTol << std::defaultfloat << "\n";
