...
namespace OpenMagnetics {
    ...
    std::string PowerFactorCorrection::generate_ngspice_circuit(double inductance,
        ...
        // Get operating parameters - use nominal if available, otherwise use minimum
        double vinRms;
        if (_inputVoltage.get_nominal().has_value()) {
            vinRms = _inputVoltage.get_nominal().value();
        } else {
            vinRms = resolve_dimensional_values(_inputVoltage, DimensionalValues::MINIMUM);
        }
    ...
    PfcSimulationWaveforms PowerFactorCorrection::simulate_and_extract_waveforms(double inductance,
        ...
        // Operating parameters - use nominal if available, otherwise use minimum
        double vinRms;
        if (_inputVoltage.get_nominal().has_value()) {
            vinRms = _inputVoltage.get_nominal().value();
        } else {
