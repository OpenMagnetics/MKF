...
public:
    ...
    MAS::Topology topology_kind() const override { return MAS::Topology::SINGLE_SWITCH_FORWARD_CONVERTER; }
    ...
    double get_total_reflected_secondary_current(TopologyExcitation forwardOperatingPoint, std::vector<double> turnsRatios, double rippleRatio=1);

    OperatingPoint process_operating_points_for_input_voltage(double inputVoltage, TopologyExcitation outputOperatingPoint, std::vector<double> turnsRatios, double inductance, double mainOutputInductance);
...
class AdvancedSingleSwitchForward : public SingleSwitchForward {
private:
...
public:
    ...
    AdvancedSingleSwitchForward(const json& j);
    ...
    // and would defeat the "Advanced" override semantics.
    DesignRequirements process_design_requirements() override;
...
void from_json(const json & j, AdvancedSingleSwitchForward & x);
void to_json(json & j, const AdvancedSingleSwitchForward & x);

inline void from_json(const json & j, AdvancedSingleSwitchForward& x) {
...
inline void to_json(json & j, const AdvancedSingleSwitchForward & x) {
    j = json::object();
