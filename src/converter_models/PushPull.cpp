...
namespace OpenMagnetics {
    ...
    OperatingPoint PushPull::process_operating_points_for_input_voltage(double inputVoltage, const PushPullOperatingPoint& outputOperatingPoint, const std::vector<double>& turnsRatios, double inductance, double outputInductance) {
        ...
        // Primary "average" / "peak" reflect the model's piecewise-linear
        // primary-current waveform during conduction (see waveform
        // construction below at lines 117–154): both endpoints sit at
        // (minimumSecondaryCurrent / N) ± ΔImag/2, so avg = midpoint and
        // peak = top endpoint.
        lastDutyCycle              = t1 * switchingFrequency;
        lastSwitchingFrequency     = switchingFrequency;
        lastPrimaryAverageCurrent  = (minimumPrimaryCurrent + maximumPrimaryCurrent) / 2.0;
    ...
    collect_input_voltages(get_input_voltage(), inputVoltages, inputVoltagesNames_);
        ...
        double riseTime = cfg.pwmRise;
        double fallTime = cfg.pwmFall;
        ...
        // PULSE timing emitted in scientific notation so sub-µs values
        // aren't silently rounded by std::fixed (precision 6) to the
        // wrong duty cycle / Fs.
        circuit << "* PWM Control Signals (alternating, non-overlapping)\n";
        circuit << "Vpwm1 pwm_ctrl1 0 PULSE(0 " << cfg.pwmHigh << " 0 "
                << std::scientific << riseTime << " " << fallTime << " "
                << tOn << " " << period << std::fixed << ")\n";
        circuit << "Vpwm2 pwm_ctrl2 0 PULSE(0 " << cfg.pwmHigh << " "
                << std::scientific << (period/2) << " " << riseTime << " " << fallTime << " "
                << tOn << " " << period << std::fixed << ")\n";
        circuit << ".model SW1 SW VT=" << cfg.swModelVT << " VH=" << cfg.swModelVH
                << " RON=" << cfg.swModelRON << " ROFF=" << std::scientific << cfg.swModelROFF
                << std::fixed << "\n\n";
        ...
        // dsec_top (verified empirically — see SpiceSimulationConfig
        // registry comment for PUSH_PULL_CONVERTER). Damping is
        // provided by the 1 MΩ convergence helpers below plus the
        // secondary RC diode snubbers, both retained from the original
        // hand-tuned netlist.
        ...
        circuit << ".model DIDEAL D(IS=" << std::scientific << cfg.diodeIS
                << " RS=" << cfg.diodeRS << std::fixed << " CJO=1e-12)\n";
        ...
        if (outputCurrent <= 0.0) {
            throw std::invalid_argument(
                "PushPull::generate_ngspice_circuit: outputCurrent must be > 0 "
        ...
        // commutation of two diodes and two switches per period.
