...
namespace OpenMagnetics {
    ...
    std::vector<SingleSwitchForwardTopologyWaveforms> SingleSwitchForward::simulate_and_extract_topology_waveforms(
        const std::vector<double>& turnsRatios,
        double magnetizingInductance) {
        
        std::vector<SingleSwitchForwardTopologyWaveforms> allWaveforms;
        ...
        for (size_t inputVoltageIndex = 0; inputVoltageIndex < inputVoltages.size(); ++inputVoltageIndex) {
            ...
            for (size_t opIndex = 0; opIndex < get_operating_points().size(); ++opIndex) {
                ...
                std::cout << "DEBUG [SingleSwitchForward]: Setting config.steadyStateCycles = " << config.steadyStateCycles << std::endl;
                config.keepTempFiles = false;
                ...
                SingleSwitchForwardTopologyWaveforms waveforms;
                ...
                for (size_t i = 0; i < simResult.waveformNames.size(); ++i) {
                    ...
                    } else if (name == "demag_in") {
                        waveforms.demagVoltage = waveform.get_data();
                    } else if (name == "vdemag_sense#branch") {
                        waveforms.demagCurrent = waveform.get_data();
                    }
