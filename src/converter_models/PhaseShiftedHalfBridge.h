...
public:
    ...
    MAS::Topology topology_kind() const override { return MAS::Topology::PHASE_SHIFTED_HALF_BRIDGE_CONVERTER; }
    ...
    // D(IS=1e-12 RS=0.05 BV=1000 IBV=1e-12)).
...
inline void from_json(const json& j, AdvancedPshb& x) {
    ...
    x.set_operating_points(j.at("operatingPoints").get<std::vector<MAS::PshbOperatingPoint>>());
