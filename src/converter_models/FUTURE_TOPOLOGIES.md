...
| `IsolatedBuck.cpp` | **DAB-quality** — 3 ref designs (LM5160 24→12V Fly-Buck, LM5017 48→5V, LM5160-Q1 60→12V automotive) with iPri NRMSE ≤ 0.12 vs SPICE; K=1 coupled-inductor physics: primary winding current is now a PIECEWISE 4-sample waveform (not symmetric triangle) capturing the off-time reflected-secondary step (i_pri = i_mag − Σ i_sec/n during freewheel, i_pri = i_mag during on-time); SPICE Kpri_sec and Ksec_sec both K=1 (matches Flyback and prevents secondary-diode forward-bias breakdown when Vout_sec ≈ Vd) |
| `IsolatedBuckBoost.cpp` | **DAB-quality** — 3 ref designs (MAX17498A 12→5V, LM5180 SNVA827 24→12V, Erickson §6.1.3 24→36V step-up) with iPri NRMSE 0.8–1.6 % vs SPICE (gate 15 %); refactored to canonical Fly-Buck-Boost (TI SNVAA84) inverting topology — Dpri anode at vpri_rect/cathode at pri_in, Cpri IC=−|Vout|, `.nodeset v(vpri_out)=−|Vout|`; previous wiring left Dpri permanently reverse-biased (Vout drifted, all power through Dsec, DCM @ NRMSE 17–36 %); analytical primary-current average now correctly /(1−D) per Erickson §5.2 buck-boost charge balance; Bimag = I(Lpri)+ΣI(Lsec)/n probe gives continuous triangle (no piecewise needed unlike IsolatedBuck) |
