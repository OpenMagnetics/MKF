    ...
    // ---- Store extra component waveforms ----
    {
        ...
        for (int k = 0; k <= N_samples; ++k) {
            if (k <= n_power) {
                ...
                double frac = (n_power > 0) ? (double)k / n_power : 0.0;
        ...
        // Negative half mirrors positive (rectified)
        for (int k = 1; k <= N_samples; ++k) {
        ...
        // Series inductor uses primary waveforms
        Waveform lrVWfm;
    ...
    size_t numberOfPeriods)
{
    ...
    for (size_t inputVoltageIndex = 0; inputVoltageIndex < inputVoltages.size(); ++inputVoltageIndex) {
        for (size_t opIndex = 0; opIndex < get_operating_points().size(); ++opIndex) {
            ...
            wf.set_input_voltage(getWaveform("v(pri_in)"));
            wf.set_input_current(getWaveform("i(vpri_sense)"));
            
            if (!turnsRatios.empty()) {
                wf.get_mutable_output_voltages().push_back(getWaveform("v(out_rect)"));
                wf.get_mutable_output_currents().push_back(getWaveform("i(vout_sense)"));
            }
