...
namespace OpenMagnetics {
    ...
    ElectromagneticParameter InputsWrapper::get_induced_voltage(OperationPointExcitation& excitation, double magnetizingInductance) {
        auto constants = Constants();
        Waveform sourceWaveform = excitation.get_current().value().get_waveform().value();
        std::vector<double> source = sourceWaveform.get_data();
        bool isWaveformSampled = !bool(sourceWaveform.get_time()) || source.size() == constants.number_points_samples_waveforms;
        ...
        voltageElectromagneticParameter.set_processed(get_processed_data(voltageElectromagneticParameter, sampledWaveform, true));
    ...
    ElectromagneticParameter InputsWrapper::get_magnetizing_current(OperationPointExcitation& excitation, Waveform sampledWaveform, double magnetizingInductance){
        double dcCurrent = 0;
    ...
    ElectromagneticParameter InputsWrapper::get_magnetizing_current(OperationPointExcitation& excitation, double magnetizingInductance){
        double dcCurrent = 0;
        if (excitation.get_current()) {
            dcCurrent = excitation.get_current().value().get_processed().value().get_offset();
        }
        ...
        for (auto &excitation : operationPoint.get_mutable_excitations_per_winding()){
            ...
            if (excitation.get_voltage()) {
                ...
                if (!excitation.get_magnetizing_current()) {
                    excitation.set_magnetizing_current(get_magnetizing_current(excitation, sampledWaveform, magnetizingInductance));
                }
    ...
    InputsWrapper InputsWrapper::create_quick_operation_point(double frequency, double magnetizingInductance, double temperature, 
        ...
        auto voltage = inputs.get_mutable_operation_points()[0].get_mutable_excitations_per_winding()[0].get_voltage().value();
        ...
        excitation.set_current(InputsWrapper::add_offset_to_excitation(magnetizingCurrentElectromagneticParameter, dcCurrent, frequency));
