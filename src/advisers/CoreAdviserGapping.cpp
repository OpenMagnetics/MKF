...
// NOTE (July 2026 health pass): the dead legacy sizing paths were removed — the free
// function add_gapping (which sized the energy gap with RAW Bsat, diverging from the
// derated ceiling calculate_gapping_constraints uses) plus the never-called analytical
// optimizer trio calculate_gap_cost_analytical / optimize_gap_and_turns_binary_search /
// add_gapping_and_turns_analytical. Recover from git history if ever needed.
