#!/usr/bin/env python3
"""
CircuiTikz Buck Converter Schematic Generator

This tool generates a Buck converter circuit diagram using CircuiTikz (LaTeX)
and exports it to PDF, PNG, or SVG formats.

Usage:
    python circuitikz_buck.py [--output OUTPUT] [--format FORMAT] [--show-values]
...
class BuckConverterParams:
    """Parameters for the Buck converter circuit"""
    input_voltage: float = 24.0          # V
...
def generate_buck_circuitikz(params: Optional[BuckConverterParams] = None, 
                              show_values: bool = True) -> str:
    """
    Generate CircuiTikz LaTeX code for a Buck converter
...
\coordinate (sw_in) at (3, 3);
\coordinate (sw_node) at (5, 3);
\coordinate (ind_out) at (8, 3);
...
\draw (1.5, 3) to[C, l="""
    latex += cin_label
    latex += r"""] (1.5, 0);
\draw (1.5, 0) -- (vin_minus);

% High-side MOSFET switch
\draw (1.5, 3) -- (sw_in);
\draw (sw_in) to[Tnmos, n=mosfet] (sw_node);
\node[left, font=\footnotesize] at (mosfet.G) {PWM};

% Freewheeling diode (cathode up to sw_node, anode to ground)
\draw (gnd_sw) to[D, l_=$D$] (sw_node);

% Inductor
\draw (sw_node) to[L, l_="""
    latex += l_label
    latex += r"""] (ind_out);
...
def compile_latex(latex_content: str, output_path: Path, output_format: str = "pdf") -> bool:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        tmpdir = Path(tmpdir)
        tex_file = tmpdir / "buck_circuit.tex"
        pdf_file = tmpdir / "buck_circuit.pdf"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Buck converter circuit diagram using CircuiTikz"
    )
    parser.add_argument(
        "-o", "--output",
        type=str,
        default="buck_circuit",
    ...
    params = BuckConverterParams(
        ...
        duty_cycle=args.vout / args.vin  # Calculate from Vout/Vin
    )
    
    # Generate LaTeX
    show_values = not args.no_values
    latex_content = generate_buck_circuitikz(params, show_values)
